...
def send():
	while True:
        users = User.query.filter_by(next_feed=datetime.today()).all()
        if len(users) != 0:
        for user in users:
            feed=[]
            print(user)
            while len(feed)<2:
                Range = Feed.query.filter_by(tags=user.tags).all()#(Data.execute("""select feed_No from feed where tags=?""",[user[7]])).fetchall()
                Range=[i.feed_no for i in Range]
                ans = check(user.id,Range,user.tag)
                if ans == "stop":
                    break
                elif  ans > 0:
